  ...
  <div class="app-container">
    ...
    <el-card style="margin-top: 20px">
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="编号" align="center" fixed="left" min-width="60">
          <template slot-scope="{ row }">
            <span>{{ row.id }}</span>
          </template>
        </el-table-column>
        <el-table-column label="头像" align="center" :show-overflow-tooltip="true" min-width="60">
          <template slot-scope="{ row }">
            <el-avatar size="large" :src="row.headimgurl"></el-avatar>
          </template>
        </el-table-column>
        <el-table-column label="订单号" align="center" min-width="145">
          <template slot-scope="{ row }">
            <el-popover placement="top" title="订单信息" width="250" trigger="hover">
              <p>
              ...
              <p>
                <span>支付时间：{{row.pay_time}}</span>
              </p>
              <span slot="reference">{{ row.order_no }}</span>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column label="车牌号" align="center" :show-overflow-tooltip="true">
          <template slot-scope="{ row }">
        ...
        <el-table-column label="消费项目" align="center" :show-overflow-tooltip="true">
          <template slot-scope="{ row }">
            <span>{{ row.num }}</span>
          </template>
        </el-table-column>
        <el-table-column label="实付金额" align="center" :show-overflow-tooltip="true" min-width="80">
          <template slot-scope="{ row }">
            <span>{{ row.price }}</span>
          </template>
        </el-table-column>
        <el-table-column label="抵扣积分" align="center" :show-overflow-tooltip="true" min-width="80">
          <template slot-scope="{ row }">
            <span>{{ row.discount_price }}</span>
          </template>
        </el-table-column>
        <el-table-column label="订单金额" align="center" :show-overflow-tooltip="true" min-width="80">
          <template slot-scope="{ row }">
        ...
        <el-table-column label="状态" align="center" :show-overflow-tooltip="true" min-width="60">
          <template slot-scope="{ row }">
            <el-tag :type="row.status | typeFilter" effect="dark">{{ row.status | statusFilter }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="备注" align="center" :show-overflow-tooltip="true" min-width="140">
          <template slot-scope="{ row }">
        ...
        <el-table-column label="操作" align="center" min-width="120">
          <template slot-scope="{ row }">
            ...
            <el-button size="mini" type="success" @click="handleDel(row.id)">明细</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
export default {
  ...
  filters: {
    // 订单状态 1 确认成功 服务中 2 已下单 待确认 3 已退款  4 退款失败  5服务   已完成 6 已取消
    statusFilter(status) {
      const statusMap = {
        1: '服务中',
        2: '待支付',
  ...
  created() {
    this.getList()
    this.getSelectFlimroomOptions()
  },
  methods: {
    //提交订单备注
    submitProData() {
      setSaleOrderRemark(this.form).then(res => {
        this.$message({
          message: '提交成功',
          type: 'success'
        })
    ...
    handleDel(id) {
      this.$confirm('此操作将删除订单信息此操作比较敏感, 是否继续?', '提示', {
        ...
        .then(() => {
          delQueueOrder({ id: id }).then(response => {
            this.$message({
              type: 'success',
              message: '操作成功!'
            })
